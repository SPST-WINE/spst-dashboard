...
export default function QuotazioniListPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r => {
      ...
      return txt.includes(needle) || String(r.id).toLowerCase().includes(needle);
    });
  }, [rows, q]);
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        {filtered.map((r) => {
          ...
          const stato = f['Stato'];
          const spedLinkId = Array.isArray(f['Spedizione_Creata']) && f['Spedizione_Creata'][0];
          ...
          return (
            <div key={r.id} className="rounded-xl border bg-white p-4 shadow-sm">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                {spedLinkId ? (
                  <a href="/dashboard/spedizioni" className="rounded-lg border px-3 py-1.5 text-sm hover:bg-slate-50">
